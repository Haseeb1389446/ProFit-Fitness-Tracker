    ...
    return (
        <div className="container mx-auto p-4 max-w-4xl">
            ...
            <div className="flex gap-4 mb-6 border-b border-gray-700">
                <button
                    className={`pb-2 px-4 font-semibold ${activeTab === 'log' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-400 hover:text-white'}`}
                    onClick={() => setActiveTab('log')}
                >
                    Log Meal
                </button>
                <button
                    className={`pb-2 px-4 font-semibold ${activeTab === 'history' ? 'text-red-500 border-b-2 border-red-500' : 'text-gray-400 hover:text-white'}`}
            ...
            {activeTab === 'log' ? (
                ...
                <div className="space-y-6">
                    {/* Filters */}
                    <div className="bg-gray-800 p-4 rounded-lg grid grid-cols-1 md:grid-cols-3 gap-4">
                        <input
                            type="text"
                    ...
                    <div className="grid gap-4">
                        {loading ? <p className="text-gray-400">Loading...</p> : logs.map(log => (
                            <div key={log._id} className="bg-theme-dark p-4 rounded-lg flex justify-between items-start border border-gray-700 hover:border-gray-500 transition-colors">
                                <div>
                                    ...
                                    <div className="space-y-1">
                                        {log.foodItems.map((item, idx) => (
                                            <div key={idx} className="text-gray-300 text-sm">
                                                {item.name} ({item.quantity}) - <span className="text-gray-100">{item.calories} kcal</span>
                                            </div>
                                        ))}
                                    </div>
                        ...
                        {!loading && logs.length === 0 && (
                            <div className="text-center text-gray-500 mt-10">No meals found.</div>
                        )}
                    </div>
                </div>
            )}
